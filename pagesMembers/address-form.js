...
const _sfc_main = /* @__PURE__ */ common_vendor.defineComponent({
  ...
  setup(__props) {
    ...
    const getMemberAddressByIdData = async () => {
      if (query.id) {
        const rs = await services_address.getMemberAddressByIdAPI(query.id);
        Object.assign(form.value, rs.result);
      }
    };
    const rules = {
      receiver: {
    ...
    const onSubmit = async () => {
      ...
      try {
        ...
        if (query.id) {
          const rs = await services_address.putMeberAPI(query.id, form.value);
        } else {
        ...
        common_vendor.index.showToast({ icon: "error", title: "请保存完整信息" });
      }
    };
    ...
    return (_ctx, _cache) => {
      return common_vendor.e({
        ...
        q: common_vendor.sr(formRef, "5779cf24-0", {
          "k": "formRef"
        }),
        r: common_vendor.p({
          rules,
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__file", "G:/gitCode/heima-shop/src/pagesMembers/address-form.vue"]]);
